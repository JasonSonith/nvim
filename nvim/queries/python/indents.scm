; Copy of nvim-treesitter's python indents.scm minus its ERROR-node
; try/except pattern, which fails to compile against the master-branch
; parser pinned in treesitter.lua ("Impossible pattern"). A broken query
; makes indentexpr error out, so every new line indents to column 0.
[
